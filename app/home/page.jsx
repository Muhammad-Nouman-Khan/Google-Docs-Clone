...
import { MoreVert, Folder } from "@mui/icons-material";
...
export default function AuthenticatedApp() {
  ...
  const userDocsRef = user
    ? collection(db, "userDocs", user.email, "docs")
    : null;
  const [snapshot, loading, error] = useCollection(
    user ? query(userDocsRef, orderBy("timestamp", "desc")) : null
  );

  const createDocument = async () => {
    if (!input || !userDocsRef) return;
    try {
      ...
      <section className="bg-[#F8F9FA] pb-10 px-10">
        <div className="max-w-3xl mx-auto">
          ...
          <div>
            ...
            >
              <Image src="/plus.png" fill alt="New document" />
            </div>
